...
//   let note
  ...
  function exportInfoData(year, month, day) {
	// get_keywords()  
	get_noteInfo(year, month, day)
	get_keywords()

	yearInfo = year
    monthInfo = month
    dayInfo = day
	console.log(yearInfo, monthInfo, dayInfo, noteInfo, keywords)
	// console.log(day)
	// console.log(worked)
...
<Modal2>
  <Info {yearInfo} {monthInfo} {dayInfo} {noteInfo} {keywords}/>
   <!-- {noteInfo} {keywords}/> -->
</Modal2>
	  ...
	  year === today.year}>
		...
		<button 
		  ...
		  {#if 
			...
			worked.year === year && worked.month === (monthIndex+1) && 
