...
function post_videoinfo(event) {
  ...
  // console.log(video)
  fastapi('put', '/api/video/input_videoinfo', videoInfo )
}

function getVideoInfo() {
  let url = '/api/video/detail/' + videoInfo.id
  let params = {
...
export let videoInfo = {
  ...
  <div class="ratio ratio-16x9">
    <video controls muted src="{encodeURIComponent('/video/' + videoInfo.dbid)}" async/>
  </div>
